...
void Converter::ReadAssetFile(wstring file)
{
	...
	std::filesystem::path p = std::filesystem::path(fileStr);
...
{
	wstring finalPath = _modelPath + savePath + L".xml";
	ReadMaterialData();
	...
	for (int32 i = 0; i < _scene->mNumMeshes; ++i)
	{
		...
		for (uint32 b = 0; b < srcMesh->mNumBones; ++b)
		{
			...
			for (uint32 w = 0; w < srcMeshBone->mNumWeights; ++w)
			{
				...
				// TODO
				tempVertexBoneWeight[index].AddWeight(boneIndex, weight);
			}
		}
		...
		for (uint32 v = 0; v < tempVertexBoneWeight.size(); ++v)
		{
			...
			asBlendWeight blendWeight = tempVertexBoneWeight[v].GetBlendWeight();
...
shared_ptr<asAnimationNode> Converter::ParseAnimationNode(shared_ptr<asAnimation> animation, aiNodeAnim* srcNode)
{
	...
	for (uint32 k = 0; k < keyCount; ++k)
	{
		...
		if (found == true)
		{
			node->keyframe.emplace_back(frameData);
		}
	}
	...
	if (node->keyframe.size() < animation->frameCount)
	{
		...
		{
			node->keyframe.emplace_back(keyFrame);
		}
	}
