...
RawBuffer::RawBuffer(void* inputData, int32 inputByte, int32 outputByte)
...
void RawBuffer::CreateSRV()
{
	...
	srvDesc.Format = DXGI_FORMAT_R32_TYPELESS; // 쉐이더에서 알아서 하기
	srvDesc.ViewDimension = D3D11_SRV_DIMENSION_BUFFEREX; // SRV_FLAG_RAW
	srvDesc.BufferEx.Flags = D3D11_BUFFEREX_SRV_FLAG_RAW;
	srvDesc.BufferEx.NumElements = desc.ByteWidth / 4; // 전체 데이터 갯수
...
void RawBuffer::CreateResult()
{
	...
	desc.BindFlags = D3D11_USAGE_DEFAULT; // UAV가 연결되려면, USAGE는 DEFAULT이여야함.
	desc.MiscFlags = 0;
